...
const LocationSearch: React.FC<LocationSearchProps> = ({ onSelect, placeholder = "Type a city...", className = "", initialValue = "", disabled = false }) => {
  ...
  useEffect(() => {
    const delayDebounce = setTimeout(() => {
      if (query.length > 2 && !disabled) { fetchSuggestions(query); } else { setSuggestions([]); setShowDropdown(false); }
    }, 500);
    ...
    <div className="relative w-full">
      <div className="relative">
        ...
        <input 
            ...
            onFocus={() => query.length > 2 && !disabled && setShowDropdown(true)} 
            placeholder={placeholder} 
            className={`w-full p-2 pl-10 border rounded-md bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 ${disabled ? 'cursor-not-allowed bg-gray-200' : ''} ${className}`}
            disabled={disabled}
        />
      </div>
...
const EditTravelRequestModal: React.FC<EditTravelRequestModalProps> = ({ isOpen, onClose, request, onUpdate, statusName }) => {
    ...
    const [isInTransitMode, setIsInTransitMode] = useState(false);

    useEffect(() => {
        setIsInTransitMode(statusName === 'InTransit');
    }, [statusName]);
    ...
    useEffect(() => {
        const fetchProjectCodes = async () => {
          ...
          } catch (error) { console.error("Failed to fetch project codes:", error); setProjectCodesError("Failed to load codes. You can enter one manually."); } finally { setProjectCodesLoading(false); }
        };
        fetchProjectCodes();
    }, []);
    ...
    useEffect(() => {
        if (request) {
            // FIX: This function is corrected to be type-safe.
            const getTravelModeName = (id: number): 'Flight' | 'Train' | 'Bus' | 'Cab' => {
                const map: { [key: number]: 'Flight' | 'Train' | 'Bus' | 'Cab' } = {
                    1: 'Flight',
                    2: 'Train',
                    3: 'Bus',
                    4: 'Cab'
                };
                return map[id] || 'Flight';
            };
            const splitISODate = (isoDate: string | null) => {
                if (!isoDate) return { date: '', time: '' };
                try { const dateObj = parseISO(isoDate); return { date: format(dateObj, 'yyyy-MM-dd'), time: format(dateObj, 'HH:mm') }; } catch (error) { return { date: '', time: '' }; }
            };
            ...
            setFormData({
                travelType: request.isInternational ? 'International' : 'Domestic', tripType: request.isRoundTrip ? 'Round Trip' : 'One Way',
                source: { label: sourceDisplay }, destination: { label: destinationDisplay }, sourceText: sourceDisplay, destinationText: destinationDisplay,
                projectCode: request.projectCode || '', departureDate: outbound.date, departureTime: outbound.time, departureArrivalDate: outboundArrival.date, departureArrivalTime: outboundArrival.time,
                returnDepartureDate: returnDep.date, returnDepartureTime: returnDep.time, returnArrivalDate: returnArr.date, returnArrivalTime: returnArr.time,
                modeOfTransport: getTravelModeName(request.travelModeId), purpose: request.purposeOfTravel || '', comments: request.comments || '',
                requiresAccommodation: request.isAccommodationRequired, requiresPickup: request.isPickUpRequired, pickupLocation: request.pickUpPlace || '',
                requiresDropoff: request.isDropOffRequired, dropoffLocation: request.dropOffPlace || '',
                requiresFoodPreference: request.isVegetarian || !!request.foodComment, foodPreference: request.isVegetarian ? 'veg' : 'non-veg', foodPreferenceComment: request.foodComment || '', 
            });
        }
    }, [request]);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (!formData.comments || formData.comments.trim() === '') {
            alert('Please provide a reason for the edit in the "Additional Comments" field.');
        ...
        const createISODate = (date: string, time: string) => { if (!date) return null; return new Date(`${date}T${time || '00:00'}:00`).toISOString(); };
        ...
        const mappedData = {
            travelModeId: getTravelModeId(formData.modeOfTransport), isInternational: formData.travelType === 'International', isRoundTrip: formData.tripType === 'Round Trip',
            projectCode: formData.projectCode, sourcePlace: sourceData.place, sourceCountry: sourceData.country, destinationPlace: destinationData.place, destinationCountry: destinationData.country,
            outboundDepartureDate: createISODate(formData.departureDate, formData.departureTime), outboundArrivalDate: createISODate(formData.departureArrivalDate, formData.departureArrivalTime),
            ...
            isAccommodationRequired: formData.requiresAccommodation, isDropOffRequired: formData.requiresDropoff, dropOffPlace: formData.requiresDropoff ? formData.dropoffLocation : "",
            isPickUpRequired: formData.requiresPickup, pickUpPlace: formData.requiresPickup ? formData.pickupLocation : "", comments: formData.comments, purposeOfTravel: formData.purpose,
            isVegetarian: formData.requiresFoodPreference ? formData.foodPreference === 'veg' : false, foodComment: formData.requiresFoodPreference ? formData.foodPreferenceComment : "",
            attendedCCT: request?.attendedCCT ?? true, ldCertificatePath: request?.ldCertificatePath ?? "string"
        };
    ...
    const renderSegmentedButton = (name: 'travelType' | 'tripType', value: string, label: string) => ( <button type="button" onClick={() => setFormData(prev => ({...prev, [name]: value as any}))} disabled={isInTransitMode} className={`px-4 py-2 text-sm font-medium border first:rounded-l-md last:rounded-r-md focus:z-10 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors ${formData[name] === value ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-700 hover:bg-gray-50'} ${isInTransitMode ? 'cursor-not-allowed opacity-70' : ''}`}>{label}</button>);
    ...
    const IconInput = ({ icon: Icon, type, name, value, onChange, disabled = false, ...props }: any) => ( <div className="relative"><Icon className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 h-5 w-5 pointer-events-none" /><input type={type} name={name} value={value} onChange={onChange} disabled={disabled} className={`w-full p-2 pl-10 border rounded-md bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 ${disabled ? 'cursor-not-allowed bg-gray-200' : ''}`} {...props} /></div>);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60" onClick={onClose}>
            <div className="relative bg-gray-50 rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
                <div className="sticky top-0 bg-white border-b p-4 flex justify-between items-center z-10">
                    <h2 className="text-xl font-bold text-gray-800">Edit Travel Request</h2>
                    {isInTransitMode && <span className="text-sm font-semibold text-orange-600 bg-orange-100 px-3 py-1 rounded-full">In-Transit Mode</span>}
                    <button onClick={onClose} className="p-2 rounded-full text-gray-500 hover:bg-gray-200 hover:text-gray-800 transition-colors"><X className="w-6 h-6" /></button>
                </div>
                
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                    <div className="bg-white p-5 rounded-lg border space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6"><div><label className="block text-sm font-medium text-gray-700 mb-1">Travel Type</label><div className="inline-flex rounded-md shadow-sm">{renderSegmentedButton('travelType', 'Domestic', 'Domestic')}{renderSegmentedButton('travelType', 'International', 'International')}</div></div><div><label className="block text-sm font-medium text-gray-700 mb-1">Trip Type</label><div className="inline-flex rounded-md shadow-sm">{renderSegmentedButton('tripType', 'One Way', 'One Way')}{renderSegmentedButton('tripType', 'Round Trip', 'Round Trip')}</div></div></div>
                    </div>
                    <div className="bg-white p-5 rounded-lg border space-y-4">
                        <h3 className="font-semibold text-gray-800 text-lg">Travel Details</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div><label className="block text-sm font-medium text-gray-700 mb-1">Source Location</label><LocationSearch onSelect={(loc) => setFormData(p => ({ ...p, source: loc, sourceText: loc.label || '' }))} initialValue={formData.sourceText} placeholder="Search for source..." disabled={isInTransitMode} /></div>
                        ...
                        <div>
                            ...
                            <Select inputId="projectCode" options={projectCodesList} value={projectCodesList.find(o => o.value === formData.projectCode) || null} onChange={(o) => setFormData(p => ({ ...p, projectCode: o ? o.value : '' }))} placeholder="Select or type..." isClearable isLoading={projectCodesLoading} isDisabled={projectCodesLoading || isInTransitMode} />
                        </div>
                    </div>
                    <div className="bg-white p-5 rounded-lg border space-y-4">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                            <div><label className="block text-sm font-medium text-gray-700 mb-1">Departure Date *</label><IconInput icon={Calendar} type="date" name="departureDate" value={formData.departureDate} onChange={handleChange} disabled={isInTransitMode} /></div>
                            <div><label className="block text-sm font-medium text-gray-700 mb-1">Time</label><IconInput icon={Clock} type="time" name="departureTime" value={formData.departureTime} onChange={handleChange} disabled={isInTransitMode} /></div>
                            <div><label className="block text-sm font-medium text-gray-700 mb-1">Arrival Date</label><IconInput icon={Calendar} type="date" name="departureArrivalDate" value={formData.departureArrivalDate} onChange={handleChange} disabled={isInTransitMode} /></div>
                            <div><label className="block text-sm font-medium text-gray-700 mb-1">Time</label><IconInput icon={Clock} type="time" name="departureArrivalTime" value={formData.departureArrivalTime} onChange={handleChange} disabled={isInTransitMode} /></div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white p-5 rounded-lg border space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                <label htmlFor="comments" className="block text-sm font-medium text-gray-700 mb-1">Additional Comments <span className="text-red-500">*</span></label>
                                <textarea name="comments" id="comments" rows={4} value={formData.comments} onChange={handleChange} className="w-full p-2 border rounded-md bg-gray-50" placeholder="Provide a reason for this edit..."></textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white p-5 rounded-lg border">
                        ...
                        <div className="space-y-4">
                            <div className="flex items-center"><input type="checkbox" id="requiresAccommodation" name="requiresAccommodation" checked={formData.requiresAccommodation} onChange={handleChange} disabled={isInTransitMode} className="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" /><label htmlFor="requiresAccommodation" className="ml-2 block text-sm text-gray-900">Requires Accommodation</label></div>
                            <div>
                                <div className="flex items-center"><input type="checkbox" id="requiresPickup" name="requiresPickup" checked={formData.requiresPickup} onChange={handleChange} disabled={isInTransitMode} className="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" /><label htmlFor="requiresPickup" className="ml-2 block text-sm text-gray-900">Requires Pickup</label></div>
                                {formData.requiresPickup && <div className="pl-6 mt-2"><label className="block text-sm font-medium text-gray-700 mb-1">Pickup Location</label><LocationSearch onSelect={(loc) => setFormData(p => ({ ...p, pickupLocation: loc.label || '' }))} initialValue={formData.pickupLocation} placeholder="Search for pickup location..." disabled={isInTransitMode} /></div>}
                            </div>
                            <div>
                            ...
                            <div>
                                <div className="flex items-center"><input type="checkbox" id="requiresFoodPreference" name="requiresFoodPreference" checked={formData.requiresFoodPreference} onChange={handleChange} disabled={isInTransitMode} className="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" /><label htmlFor="requiresFoodPreference" className="ml-2 block text-sm text-gray-900">Food Preference Required</label></div>
                                {formData.requiresFoodPreference && (
                                    <div className="pl-6 mt-3 space-y-3">
                                        ...
                                        <div className="flex gap-x-6">
                                            <div className="flex items-center"><input type="radio" id="veg" name="foodPreference" value="veg" checked={formData.foodPreference === 'veg'} onChange={handleChange} disabled={isInTransitMode} className="h-4 w-4 text-blue-600 border-gray-300 focus:ring-blue-500" /><label htmlFor="veg" className="ml-2 block text-sm text-gray-700">Vegetarian</label></div>
                                            <div className="flex items-center"><input type="radio" id="non-veg" name="foodPreference" value="non-veg" checked={formData.foodPreference === 'non-veg'} onChange={handleChange} disabled={isInTransitMode} className="h-4 w-4 text-blue-600 border-gray-300 focus:ring-blue-500" /><label htmlFor="non-veg" className="ml-2 block text-sm text-gray-700">Non-Vegetarian</label></div>
                                        </div>
                                        <div className="relative">
                                            ...
                                            <textarea id="foodComment" name="foodPreferenceComment" rows={3} className={`block w-full rounded-lg border border-gray-200 bg-gray-50 px-4 py-3 text-gray-700 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none ${isInTransitMode ? 'cursor-not-allowed bg-gray-200' : ''}`} placeholder="Specify any allergies or dietary restrictions..." value={formData.foodPreferenceComment} onChange={handleChange} maxLength={500} disabled={isInTransitMode} />
