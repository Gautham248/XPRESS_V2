...
import { ComponentTravelRequest, DocumentInfo, getDisplayStatusName, getStatusBadgeStyles, INDEX_TO_STATUS_MAP } from './types';
...
const TravelRequestDetails: React.FC = () => {
  ...
  const fetchTravelRequest = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await fetch(`${API_BASE_URL}/TravelRequest/${id}`);
  ...
  const handleOpenDownloadModal = async () => {
      ...
      if (travelRequestData.ticketDocumentPath) {
          docs.push({ id: `ticket_${id}`, url: `${API_BASE_URL}/TravelRequest/${id}/downloadticket`, friendlyName: getFriendlyFilename({ idType: 'TravelTicket', id, documentPath: travelRequestData.ticketDocumentPath }), docData: {} });
      }
      const response = await fetch(`${API_BASE_URL}/Documents/User/${travelRequestData.userId}`);
      if (response.ok) {
  ...
  const handleSubmitFeedback = async () => {
      await fetch(`${API_BASE_URL}/TravelRequest/${id}/travelfeedback`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ feedbackText: modalInputText, submittingUserId: userId }) });
      setFeedbackSubmitted(true);
      handleCloseModal();
  };
  const handleSubmitCloseRequest = async () => {
      await axios.put(`${API_BASE_URL}/TravelRequest/${id}/updatestatus`, { requestId: id, newStatusId: 10, userId, comments: modalInputText, actionType: "CloseRequest" });
      setRequestClosed(true);
      fetchTravelRequest();
      handleCloseModal();
  };
  const handleSubmitApproval = async () => {
      await fetch(`${API_BASE_URL}/Approvals/${id}/manager/approve`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ approvingUserId: userId, comments: modalInputText }) });
      setActionTaken(true);
      fetchTravelRequest();
      handleCloseModal();
  };
  const handleSubmitRejection = async () => {
      ...
      await fetch(`${API_BASE_URL}/Approvals/${id}/manager/reject`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ rejectingUserId: userId, comments: modalInputText }) });
      setActionTaken(true);
      fetchTravelRequest();
      handleCloseModal();
  };
  const handleSubmitCancelRequest = async () => {
      ...
      await axios.put(`${API_BASE_URL}/TravelRequest/${id}/updatestatus`, {
      requestId: id,
        ...
        <div className="space-y-6 animate-fadeIn">
            ...
            {id && (
                <TicketPreviewModal
                    ...
                    downloadUrl={`${API_BASE_URL}/TravelRequest/${id}/downloadticket`}
                />
            )}
