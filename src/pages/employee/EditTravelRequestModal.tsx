...
interface LocationSearchProps { onSelect: (location: Address & { custom?: boolean }) => void; placeholder?: string; className?: string; initialValue?: string; }
const LocationSearch: React.FC<LocationSearchProps> = ({ onSelect, placeholder = "Type a city...", className = "", initialValue = "" }) => {
  ...
  return (
    <div className="relative w-full">
      <div className="relative">
        ...
        <input type="text" value={query} onChange={(e) => setQuery(e.target.value)} onFocus={() => query.length > 2 && setShowDropdown(true)} placeholder={placeholder} className={`w-full p-2 pl-10 border rounded-md bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 ${className}`} />
      </div>
...
interface EditTravelRequestModalProps { isOpen: boolean; onClose: () => void; request: TravelRequest | null; onUpdate: (updatedData: any) => void; }
const EditTravelRequestModal: React.FC<EditTravelRequestModalProps> = ({ isOpen, onClose, request, onUpdate }) => {
    ...
    useEffect(() => {
        const fetchProjectCodes = async () => {
          ...
          try {
            // NOTE: Replace with your actual API endpoint for project codes
    ...
    useEffect(() => {
        if (request) {
            setFormData(prev => ({
                ...
                tripType: request.tripType || (request.returnDate ? 'Round Trip' : 'One Way'), 
                source: { label: request.source }, 
                destination: { label: request.destination }, 
                sourceText: request.source, 
                destinationText: request.destination, 
                projectCode: request.projectCode, 
                departureDate: request.departureDate ? format(parseISO(request.departureDate), 'yyyy-MM-dd') : '', 
                departureTime: request.departureDate ? format(parseISO(request.departureDate), 'HH:mm') : '', 
                returnDepartureDate: request.returnDate ? format(parseISO(request.returnDate), 'yyyy-MM-dd') : '', 
                returnDepartureTime: request.returnDate ? format(parseISO(request.returnDate), 'HH:mm') : '', 
                modeOfTransport: (request.transportationType as any) || 'Flight', 
                purpose: request.purpose, 
                requiresAccommodation: request.accommodationType !== 'None' && request.accommodationType !== 'Unknown', 
            }));
        }
    }, [request]);

    useEffect(() => {
        if (formData.requiresPickup && formData.source && !formData.pickupLocation) {
            setFormData(prev => ({ ...prev, pickupLocation: prev.source?.label || '' }));
        }
    }, [formData.requiresPickup, formData.source, formData.pickupLocation]);
    
    useEffect(() => {
        if (formData.travelType === 'International') {
            setFormData(prev => ({ ...prev, modeOfTransport: 'Flight' }));
        }
    }, [formData.travelType]);
    ...
    const getTravelModeId = (mode: string) => {
        switch (mode) {
            case 'Flight': return 1;
            case 'Train': return 2;
            case 'Bus': return 3;
            case 'Cab': return 4;
            default: return 0; // Default or unspecified
        }
    };
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const mappedData = {
            ...
            attendedCCT: true, // This can be replaced with actual form data
            ldCertificatePath: "string" // This can be replaced with actual form data
        };
        
        console.log("Generated API Payload:", JSON.stringify(mappedData, null, 2));
    ...
    const renderSegmentedButton = (name: 'travelType' | 'tripType', value: string, label: string) => ( <button type="button" onClick={() => setFormData(prev => ({...prev, [name]: value as any}))} className={`px-4 py-2 text-sm font-medium border first:rounded-l-md last:rounded-r-md focus:z-10 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors ${formData[name] === value ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-700 hover:bg-gray-50'}`}>{label}</button>);
    ...
    const IconInput = ({ icon: Icon, type, name, value, onChange, ...props }: any) => ( <div className="relative"><Icon className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 h-5 w-5 pointer-events-none" /><input type={type} name={name} value={value} onChange={onChange} className="w-full p-2 pl-10 border rounded-md bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500" {...props} /></div>);
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                    ...
                    <div className="bg-white p-5 rounded-lg border space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div><label className="block text-sm font-medium text-gray-700 mb-1">Source Location</label><LocationSearch onSelect={(loc) => setFormData(p => ({ ...p, source: loc }))} initialValue={formData.sourceText} placeholder="Search for source..."/></div>
                            <div><label className="block text-sm font-medium text-gray-700 mb-1">Destination</label><LocationSearch onSelect={(loc) => setFormData(p => ({ ...p, destination: loc }))} initialValue={formData.destinationText} placeholder="Search for destination..."/></div>
                        </div>
                        <div>
                            <label htmlFor="projectCode" className="block text-sm font-medium text-gray-700 mb-1">Project Code *</label>
                            <Select inputId="projectCode" options={projectCodesList} value={projectCodesList.find(o => o.value === formData.projectCode) || null} onChange={(o) => setFormData(p => ({ ...p, projectCode: o ? o.value : '' }))} placeholder="Select or type..." isClearable isLoading={projectCodesLoading} isDisabled={projectCodesLoading} noOptionsMessage={() => projectCodesError || 'No matching codes'} />
                    ...
                    <div className="bg-white p-5 rounded-lg border space-y-4">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                            <div><label className="block text-sm font-medium text-gray-700 mb-1">Departure Date *</label><IconInput icon={Calendar} type="date" name="departureDate" value={formData.departureDate} onChange={handleChange} /></div>
                            <div><label className="block text-sm font-medium text-gray-700 mb-1">Time</label><IconInput icon={Clock} type="time" name="departureTime" value={formData.departureTime} onChange={handleChange} /></div>
                            <div><label className="block text-sm font-medium text-gray-700 mb-1">Arrival Date</label><IconInput icon={Calendar} type="date" name="departureArrivalDate" value={formData.departureArrivalDate} onChange={handleChange} /></div>
                            <div><label className="block text-sm font-medium text-gray-700 mb-1">Time</label><IconInput icon={Clock} type="time" name="departureArrivalTime" value={formData.departureArrivalTime} onChange={handleChange} /></div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white p-5 rounded-lg border">
                        ...
                        <div className="space-y-4">
                            <div className="flex items-center"><input type="checkbox" id="requiresAccommodation" name="requiresAccommodation" checked={formData.requiresAccommodation} onChange={handleChange} className="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" /><label htmlFor="requiresAccommodation" className="ml-2 block text-sm text-gray-900">Requires Accommodation</label></div>
                            <div>
                                <div className="flex items-center"><input type="checkbox" id="requiresPickup" name="requiresPickup" checked={formData.requiresPickup} onChange={handleChange} className="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" /><label htmlFor="requiresPickup" className="ml-2 block text-sm text-gray-900">Requires Pickup</label></div>
                                {formData.requiresPickup && <div className="pl-6 mt-2"><label className="block text-sm font-medium text-gray-700 mb-1">Pickup Location</label><LocationSearch onSelect={(loc) => setFormData(p => ({ ...p, pickupLocation: loc.label || '' }))} initialValue={formData.pickupLocation} placeholder="Search for pickup location..." /></div>}
                            </div>
                            <div>
                            ...
                            <div>
                                <div className="flex items-center"><input type="checkbox" id="requiresFoodPreference" name="requiresFoodPreference" checked={formData.requiresFoodPreference} onChange={handleChange} className="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" /><label htmlFor="requiresFoodPreference" className="ml-2 block text-sm text-gray-900">Food Preference Required</label></div>
                                {formData.requiresFoodPreference && (
                                    <div className="pl-6 mt-3 space-y-3">
                                        ...
                                        <div className="flex gap-x-6">
                                            <div className="flex items-center"><input type="radio" id="veg" name="foodPreference" value="veg" checked={formData.foodPreference === 'veg'} onChange={handleChange} className="h-4 w-4 text-blue-600 border-gray-300 focus:ring-blue-500" /><label htmlFor="veg" className="ml-2 block text-sm text-gray-700">Vegetarian</label></div>
                                            <div className="flex items-center"><input type="radio" id="non-veg" name="foodPreference" value="non-veg" checked={formData.foodPreference === 'non-veg'} onChange={handleChange} className="h-4 w-4 text-blue-600 border-gray-300 focus:ring-blue-500" /><label htmlFor="non-veg" className="ml-2 block text-sm text-gray-700">Non-Vegetarian</label></div>
                                        </div>
                                        <div className="relative">
                                            ...
                                            <textarea id="foodComment" name="foodPreferenceComment" rows={3} className="block w-full rounded-lg border border-gray-200 bg-gray-50 px-4 py-3 text-gray-700 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none" placeholder="Specify any allergies or dietary restrictions..." value={formData.foodPreferenceComment} onChange={handleChange} maxLength={500} />
